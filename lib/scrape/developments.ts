...
const PAGE_HEADERS = { 'User-Agent': UA, 'Accept': 'text/html,application/xhtml+xml,*/*;q=0.8', 'Accept-Language': 'en-GB,en;q=0.9' };
...
const abs = (href: string, base: string): string | null => { try { return new URL(href, base).toString(); } catch { return null; } };
...
async function fetchText(url: string, timeoutMs = 12000): Promise<{ ok: boolean; status: number; html: string }> {
  try {
...
// Respect robots.txt (best-effort): fetch it once per host and honour Disallow for
// our UA or '*'. If robots is unreachable we proceed but stay polite (one site at a
// time, short timeouts). Exported for unit testing.
...
async function robotsAllows(url: string, cache: Map<string, string>): Promise<boolean> {
  try {
