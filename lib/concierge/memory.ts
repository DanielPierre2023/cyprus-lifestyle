...
const STR_KEYS: (keyof MemoryProfile)[] = ['name', 'language', 'base', 'party', 'dates', 'dietary', 'status', 'notes'];
...
export function sanitizeProfile(input: unknown): MemoryProfile {
  ...
  if (Array.isArray(o.interests)) {
    const arr = (o.interests as unknown[]).map((x) => String(x).trim()).filter(Boolean).slice(0, 8);
    if (arr.length) p.interests = arr;
  }
  return p;
}
...
export async function updateMemory(cid: string, userText: string, assistantText: string, current: MemoryProfile): Promise<void> {
  ...
  const system =
    ...
    'Allowed keys only: name, language, interests (array of short tags), base (town they are visiting or based in), party (e.g. "2 adults + 2 children"), dates, dietary, status (one of: visiting, relocating, resident), notes (one short line). ' +
