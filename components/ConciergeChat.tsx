...
export default function ConciergeChat({ locale, labels }: { locale: Locale; labels: ConciergeChatLabels }) {
  ...
  // Trip plan (item 10): saved listing slugs, persisted server-side by cid.
  const [savedSlugs, setSavedSlugs] = useState<Set<string>>(() => new Set());
  const saveTrip = (slug: string) => {
    setSavedSlugs((prev) => { const n = new Set(prev); n.add(slug); return n; });
    try { fetch('/api/concierge/saved', { method: 'POST', keepalive: true, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ cid: cidRef.current, slug, kind: 'trip', action: 'add' }) }); } catch { /* best-effort */ }
  };
